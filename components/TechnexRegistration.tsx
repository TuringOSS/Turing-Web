...
const TechnexRegistration = () => {
  ...
  useEffect(() => {
    const initThree = () => {
      ...
      const bloomPass = new UnrealBloomPass(
        ...
        0.5, // Reduced strength to reduce blurriness
        0.4,
        0.9  // Increased threshold so only bright stars glow
      );
      ...
      for (let i = 0; i < 3; i++) {
        ...
        for (let j = 0; j < starCount; j++) {
          ...
          if (colorChoice < 0.7) {
            ...
            color.setHex(0xFFD700); // Turing Yellow
          } else {
            color.setHex(0xB8860B); // Dark Golden Rod
          }
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          color2: { value: new THREE.Color(0xFFD700) }, // Turing Yellow
          opacity: { value: 0.3 }
        },
        vertexShader: `
          varying vec2 vUv;
    ...
    const createMountains = () => {
      ...
      layers.forEach((layer, index) => {
        const points = [];
        const segments = 300; // Increased segments for sharper/smoother mountain edges
        
        for (let i = 0; i <= segments; i++) {
          const x = (i / segments - 0.5) * 3000; // Wider range to cover screen better
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec3 atmosphere = vec3(1.0, 0.84, 0.0) * intensity; // Gold/Yellow atmosphere
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Dynamic color change for title (Kept Black)
      if (titleRef.current) {
        // Keep text black for the whole duration
        const r = 0;
        const g = 0;
        const b = 0;
        
        // Change text based on section
        const titles = ['TECHNEX', '2026', 'IS HERE'];
        // Use a threshold to switch text abruptly or smooth? Abrupt for now as section changes
        if (titleRef.current.innerText !== titles[newSection]) {
             titleRef.current.innerText = titles[newSection] || 'TECHNEX';
             
             // Reset animation for new text (optional, simple fade via GSAP would be better but simple text swap works)
             gsap.fromTo(titleRef.current, {opacity: 0, y: 20}, {opacity: 1, y: 0, duration: 0.5});
        }

        titleRef.current.style.color = `rgb(${r},${g},${b})`;
        // Add white glow for visibility on dark background
        titleRef.current.style.textShadow = `0 0 20px rgba(255, 255, 255, 0.5)`;
      }

       // Dynamic color change for subtitle (White -> Black)
       if (subtitleRef.current) {
        const mix = Math.min(1, progress * 1.5); 
        const val = Math.floor(255 * (1 - mix));
        ...
        const currentSubtitle = subtitles[newSection] || subtitles[0];
        const lines = subtitleRef.current.querySelectorAll('.subtitle-line');
        if (lines[0] && lines[0].textContent !== currentSubtitle.line1) {
            lines[0].textContent = currentSubtitle.line1;
            lines[1].textContent = currentSubtitle.line2;
            gsap.fromTo(subtitleRef.current, {opacity: 0}, {opacity: 1, duration: 0.5});
        }

        subtitleRef.current.style.color = `rgb(${val},${val},${val})`;
      }
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      <div className="hero-content cosmos-content">
        ...
        <div ref={subtitleRef} className="hero-subtitle cosmos-subtitle flex flex-col items-center">
...
export default TechnexRegistration;
